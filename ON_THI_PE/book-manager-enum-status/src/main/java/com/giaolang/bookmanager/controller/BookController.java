...
public class BookController {
    ...
    @GetMapping("/books")
    public String showBooksPage(@RequestParam(name = "kw", required = false) String keyword, @RequestParam(name = "status", required = false) String status, Model model, HttpSession session) {
        ...
        //convert string từ màn hình search thành biến BookStatus
        if (keyword != null && !keyword.isEmpty()) {
            BookStatus statsE = BookStatus.valueOf(status.toUpperCase());
            //có keyword rồi, search thôi
            books = service.searchBooksByTitleAndStatus(keyword, statsE);
        }
        else {
...
//    }
    ...
    @PostMapping("/save")
    public String doSave(@ModelAttribute("book") Book book, Model model, HttpSession session) {
